...
def test_oid_excludes_only_the_six_detached_fields():
    # ADR_019 / PROJECTION_SPEC.md Section 2: the content core removes EXACTLY
    # oid, signature, ml_dsa_signature, signature_key_id, signature_algorithm,
    # attestation. gap_version and supersedes are KEPT in identity.
    content_core = {
        "type": "gap:capability_declaration",
        "gap_version": "1.0",
        "tenant_id": "test",
    ...
    post_attestation = {
        **content_core,
        "oid": "sha256:old",
        "signature": "abc",
        "ml_dsa_signature": "mldsa",
        "signature_key_id": "key1",
        "signature_algorithm": "ed25519+ml-dsa-65",
        "attestation": {"payloadType": "x", "payload": "{}", "signatures": []},
    }
    oid = compute_gap_oid(post_attestation)
    assert oid.startswith("sha256:")
    # The six detached fields strip back to the pre-attestation content core.
    assert compute_gap_oid(content_core) == oid


def test_oid_adr019_keystone_matches_sraid():
    # The exact object from PROJECTION_SPEC.md Section 2.3 must yield the OID
    # the live v2 signer stamps and @synoi/sraid computes. Per ADR_020 (sraid:
    # wire-identifier migration), the version key is sraid_version (value
    # 2.0); gap_version is a distinct L3 field and stays.
    keystone = {
        ...
        "authority": {
            "grant_oid": "sha256:aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa",
            "decision": "allow",
        },
        "supersedes": "sha256:bbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbb",
    }
    assert compute_gap_oid(keystone) == (
        "sha256:d1d5d5c51d2d5f80470089ff10b8b642e19fc76db6be298f4f346616528a087a"
    )


def test_oid_keeps_gap_version_and_supersedes_in_identity():
    # ADR_019: dropping gap_version or supersedes MUST change the OID.
    content_core = {
        "type": "gap:capability_declaration",
        "gap_version": "1.0",
        "tenant_id": "test",
    ...
    base_oid = compute_gap_oid(content_core)
    without_gv = {k: v for k, v in content_core.items() if k != "gap_version"}
    without_sup = {k: v for k, v in content_core.items() if k != "supersedes"}
    assert compute_gap_oid(without_gv) != base_oid
    assert compute_gap_oid(without_sup) != base_oid
...
if __name__ == "__main__":
    ...
    test_oid_excludes_only_the_six_detached_fields()
    test_oid_adr019_keystone_matches_sraid()
    test_oid_keeps_gap_version_and_supersedes_in_identity()
