...
**`receipt()` one-liner (T22 packaging)**
- `receipt(input, options)`: builds and self-signs a `gap:decision_receipt` CDRO
  with a single call, using the operator's own Ed25519 key. Wraps the existing
  `computeGapOid` / `canonicalize` surface; introduces no second canonicalizer or
  hashing path (same 5-field exclusion set `computeGapOid` already strips).
- `generateReceiptKeyPair(keyId?)`: local-only Ed25519 keypair generation
  (`@noble/curves`, already a declared dependency). No network call.
- `verifyReceiptSignature(envelope, publicKey)`: local signature verification.
- `DEFAULT_VERIFY_URL_BASE`: `'https://oid.synoi.systems'`. Documented as
  PENDING -- `receipt()` never calls this URL; it only formats
  `${base}/r/<oid>` as a string. Configurable via `options.verifyUrlBase`
  (pass `null` to omit `verifyUrl`).
- No network call is made anywhere in this module, by design.
