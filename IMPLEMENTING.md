...
> **Normative source:** the OID content-core projection, the number rule, and the RFC 8785 rules below are specified once, in [`synoi-sraid/PROJECTION_SPEC.md`](../synoi-sraid/PROJECTION_SPEC.md) (ADR_019, CC0). Every SDK derives from that document; the reference implementation is `@synoi/sraid` (`src/oid.ts`, `src/canonicalize.ts`). This section restates it for GAP implementers and MUST yield the exact OID the live signer stamps.

The OID is the SHA-256 hash of the canonical form of the **content core**: the CDRO with exactly the **six detached-signature / envelope fields** removed before hashing, and **everything else kept**:

| Removed (produced by the signer, or the OID itself) |
| --- |
| `oid` — the projection output; it cannot be an input to itself |
| `signature` — legacy hybrid `SignatureEnvelope`; attaches after hashing |
| `ml_dsa_signature` — detached ML-DSA-65 signature; attaches after hashing |
| `signature_key_id` — signer-stamped key id |
| `signature_algorithm` — signer-stamped algorithm id |
| `attestation` — DSSE `AttestationEnvelope`; attaches after hashing |

This set is defined **semantically**: "every field the signer produces after canonicalization, plus the OID output itself." Do NOT maintain a hand-listed strip-set per language; derive it from this rule.

**`gap_version` and `supersedes` are KEPT** and hashed into identity:

- `gap_version` is in identity so a protocol downgrade is OID-detectable.
- `supersedes` is in identity because the SRAID Merkle-DAG "head proves history" property requires every lineage edge inside the hash. Superseding mints a **new** object; it never mutates the superseded object's bytes, so keeping the pointer inside identity is safe and makes the lineage edge tamper-evident.

**Step 1: Build the content core** (the full CDRO minus the six detached fields). For a freshly minted object this is simply the object before any signature or `oid` is attached, e.g.:
...
Include `gap_version` (and `supersedes`, when present) in the hashed content. Do NOT include any of the six detached fields.

**Step 2: Canonicalize.**

Canonical JSON rules (RFC 8785 JCS; see PROJECTION_SPEC.md §3 for the normative statement):
- Object keys are sorted in ascending **UTF-16 code-unit** order (RFC 8785 §3.2.3; this is JavaScript `Array.prototype.sort` default order). For BMP characters this equals code-point order; an astral character (e.g. an emoji) sorts by its leading surrogate. Non-UTF-16 host languages MUST sort on the UTF-16 encoding, not raw UTF-8 bytes or scalar values.
...
The TypeScript helper `computeGapOid(cdro)` from `@synoi/gap` does all three steps and accepts either a pre-attestation content core or a full signed envelope (it strips the six detached fields itself). For other languages, any SHA-256 library with the canonical JSON rules above produces identical OIDs.

**Worked example (the ADR_020 keystone, formerly the ADR_019 keystone under the retired `cof_version` key).** This content core:
...
canonicalizes with `gap_version` and `supersedes` INCLUDED and hashes to:

```
sha256:d1d5d5c51d2d5f80470089ff10b8b642e19fc76db6be298f4f346616528a087a
```

Attaching an `attestation` (and `oid`, `signature`, `ml_dsa_signature`, `signature_key_id`, `signature_algorithm`) after signing does NOT change this OID: those six fields are stripped back out by the projection. This pre-/post-attestation invariance is what lets a third party recompute the OID of a signed receipt and match the value the signer stamped.
...
2. Remove exactly the six detached fields (`oid`, `signature`, `ml_dsa_signature`, `signature_key_id`, `signature_algorithm`, `attestation`) from the object. Keep everything else, including `gap_version` and `supersedes`.
3. Canonicalize and hash the remainder (see §2.2).
4. Compare to the extracted `oid`. If they differ, the object was tampered with.
5. (Optional) Verify the signature / attestation against the gateway's published public key.
