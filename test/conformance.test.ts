...
// -- Excluded fields: computeGapOid auto-strips the six detached fields -------
//
// ADR_019 / PROJECTION_SPEC.md §2: the content-core projection removes EXACTLY
// oid, signature, ml_dsa_signature, signature_key_id, signature_algorithm,
// attestation before hashing, and KEEPS everything else including gap_version
// and supersedes. computeGapOid strips those six internally, so a pre-stripped
// payload and a post-attestation envelope produce the same OID.
//
// Helper: strip a set of fields and return a clean payload (used to prove that
// stripping a detached field before hashing matches computeGapOid's internal
// strip). NOTE: gap_version and supersedes are NOT detached fields; stripping
// them here only constructs the baseline payload that lacks them.
function stripFields(obj: Record<string, unknown>, fields: string[]): Record<string, unknown> {
  const out: Record<string, unknown> = {}
  for (const k of Object.keys(obj)) if (!fields.includes(k)) out[k] = obj[k]
  return out
}
function stripExcluded(obj: Record<string, unknown>): Record<string, unknown> {
  return stripFields(obj, ['oid', 'gap_version', 'signature', 'signature_key_id', 'supersedes'])
}
...
const V_WITH_ML_DSA = { ...V_DECL_PAYLOAD, ml_dsa_signature: 'mldsa:aabbcc' }
ok('OID: after stripping ml_dsa_signature, hash matches baseline',
   computeGapOid(stripFields(V_WITH_ML_DSA as Record<string, unknown>, ['ml_dsa_signature'])) === V_DECL_OID)

const V_WITH_SIG_ALG = { ...V_DECL_PAYLOAD, signature_algorithm: 'ed25519+ml-dsa-65' }
ok('OID: after stripping signature_algorithm, hash matches baseline',
   computeGapOid(stripFields(V_WITH_SIG_ALG as Record<string, unknown>, ['signature_algorithm'])) === V_DECL_OID)

const V_WITH_ATTESTATION = { ...V_DECL_PAYLOAD, attestation: { payloadType: 'x', payload: '{}', signatures: [] } }
ok('OID: after stripping attestation, hash matches baseline',
   computeGapOid(stripFields(V_WITH_ATTESTATION as Record<string, unknown>, ['attestation'])) === V_DECL_OID)

// computeGapOid auto-strips the six detached fields, so a full envelope
// carrying any of them produces the same OID as the pre-stripped payload.
...
ok('OID: unstripped ml_dsa_signature auto-stripped by computeGapOid -- matches baseline',
   computeGapOid(V_WITH_ML_DSA) === V_DECL_OID)
ok('OID: unstripped signature_algorithm auto-stripped by computeGapOid -- matches baseline',
   computeGapOid(V_WITH_SIG_ALG) === V_DECL_OID)
ok('OID: unstripped attestation auto-stripped by computeGapOid -- matches baseline',
   computeGapOid(V_WITH_ATTESTATION) === V_DECL_OID)

// ADR_019: gap_version and supersedes are NOW KEPT in identity. An envelope
// carrying them must produce a DIFFERENT OID from the baseline that lacks them.
const V_WITH_GAP_VERSION = { ...V_DECL_PAYLOAD, gap_version: '1.0' }
ok('OID: gap_version is KEPT in identity -- full envelope differs from baseline (ADR_019)',
   computeGapOid(V_WITH_GAP_VERSION) !== V_DECL_OID)

const V_WITH_SUPERSEDES = { ...V_DECL_PAYLOAD, supersedes: 'sha256:' + 'f'.repeat(64) }
ok('OID: supersedes is KEPT in identity -- full envelope differs from baseline (ADR_019)',
   computeGapOid(V_WITH_SUPERSEDES) !== V_DECL_OID)
