...
/**
 * Normative GAP capability names that an MCP tool or server name MUST NOT
 * shadow. The 'gap:' prefix rule (see isReservedMcpName) covers signed-type
 * collisions; this set covers dotted-taxonomy collisions with first-class
 * capabilities. This is the CANONICAL source of truth for the reserved-name
 * check: every enforcing consumer (the gateway MCP adapter, any third-party
 * gateway, audit tooling) MUST import this rather than re-declaring its own
 * copy, so the namespace-pollution guard cannot drift between implementations.
 *
 * Keep in sync with CAPABILITY_TAXONOMY.md. Extend conservatively as the
 * normative taxonomy grows.
 */
export const NORMATIVE_CAPABILITY_NAMES: ReadonlySet<string> = new Set([
  'email.send',
  'sms.send',
  'memory.write',
  'memory.read',
...
/**
 * Return a human-readable reason string if `name` is a reserved MCP tool/server
 * name that would pollute the GAP namespace, or null if the name is safe.
 *
 * A name is reserved when it is empty, starts with the reserved 'gap:' prefix
 * (case-insensitive), or collides with a NORMATIVE_CAPABILITY_NAMES entry. Per
 * the McpToolCallContext rule (above), the gateway MUST reject any
 * auto-generated capability name from an attacker-controlled tools/list
 * response that trips this predicate.
 *
 * This is the canonical predicate; enforcing consumers import it instead of
 * carrying a private copy.
 */
export function reservedMcpNameReason(name: string): string | null {
  const n = name.trim()
  if (n.length === 0) return 'empty name'
  if (n.toLowerCase().startsWith('gap:')) return `name starts with reserved 'gap:' prefix`
  if (NORMATIVE_CAPABILITY_NAMES.has(n)) return `name collides with normative capability '${n}'`
  return null
}

/** Convenience boolean form of {@link reservedMcpNameReason}. */
export function isReservedMcpName(name: string): boolean {
  return reservedMcpNameReason(name) !== null
}
